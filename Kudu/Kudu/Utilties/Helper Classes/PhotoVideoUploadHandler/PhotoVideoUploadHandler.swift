...
final class PhotoVideoUploadHandler {
    ...
    static func uploadVideoOnAws(path: URL, uploadId: String, progress: @escaping progressBlock, handler: @escaping (Bool, String?, String?) -> Void) {
        ...
        AWSUploadController.uploadTheVideoToAWS(videoUrl: path, progress: progress, completion: { (awsUploadLink, error) in
    ...
    static func uploadImageOnAws(imageUploadReqID: String, image: UIImage, compression: Double = 1, progress: @escaping (Double?) -> Void, handler: @escaping (Bool, String?, String?) -> Void ) {
        ...
        AWSUploadController.uploadTheImageToAWS(compression: compression, image: image, completion: { (awsUploadLink, error) in
            if error.isNil && awsUploadLink.isNotNil {
                debugPrint("Uploaded at \(awsUploadLink ?? "")")
                request.uploaded = true
                request.awsLink = awsUploadLink!
