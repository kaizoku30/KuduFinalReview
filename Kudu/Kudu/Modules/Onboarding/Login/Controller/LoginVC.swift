...
class LoginVC: BaseVC {
    ...
    private func initialiseVM() {
        ...
        let selectedLanguage: LanguageSelectionView.LanguageButtons = language == .ar ? .arabic : .english
        viewModel = LoginVM(selectedLang: selectedLanguage, _delegate: self)
    }
...
extension LoginVC: LoginVMDelegate {
    ...
    func socialAPIResponse(isSuccess: Bool, socialSignUpRequest: SocialSignUpRequest?, errorMsg: String?) {
        mainThread {
            if isSuccess {
                let underDevelopmentVC = DevelopmentPlaceholderVC.instantiate(fromAppStoryboard: .Onboarding)
                self.push(vc: underDevelopmentVC)
